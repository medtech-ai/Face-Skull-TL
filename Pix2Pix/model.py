...
class UNet(nn.Module):
    def __init__(self, n_layers=4, in_channels=3, features=32):
        ...
        in_sizes = [in_channels] + [features * (2 ** i) for i in range(n_layers - 1)]
...
class Critic(nn.Module):
    def __init__(self, image_size=128, conv_dim=64, n_layers=6):
        super().__init__()
        layers = []
        layers.append(SpectralNorm(nn.Conv2d(1, conv_dim, kernel_size=4, stride=2, padding=1)))
...
class StarGAN:
    def __init__(self, device, g_conv_dim=64, g_n_layers=6, image_size=128, d_conv_dim=64, d_n_layers=6, c_dim=1):
        # self.G = Generator(g_conv_dim, c_dim, g_n_layers).to(device)
        self.G = UNet(g_n_layers, c_dim, g_conv_dim).to(device)
        self.D = Critic(image_size, d_conv_dim, d_n_layers).to(device)
    ...
    def trainG(self, image, mask):
        ...
        fake_images = self.G(image)
        out_fake = self.D(fake_images)
        
        g_loss_fake = -torch.mean(out_fake)

        g_loss_rec = self.criterionL1(fake_images, mask)

        g_loss = g_loss_fake + 50 * g_loss_rec
